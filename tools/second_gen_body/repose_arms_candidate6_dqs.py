"""candidate6：以對偶四元數蒙皮（DQS）執行手臂重擺。

擁有者核定採用 DQS，為未來更大幅度的姿勢共用穩定蒙皮基礎。
目前肩角 34.2 度、肘角 54.6 度，旋轉聚焦於擺臂。
演算法依 Kavan et al. 2007《Skinning with Dual Quaternions》：
將骨骼剛體變換表為單位對偶四元數，依權重線性混合後正規化，
讓混合保留剛體變換的體積特性。

LBS 版依序旋轉肩與肘；DQS 版先組合恆等、上臂及前臂變換，
再以標準骨骼形式一次混合。彎曲關節可能增加局部體積，因此輸出
肩肘鄰域的最大膨脹量供查驗；原論文亦描述 DQS/LBS 混權的處理方向。

採用前驗證：保留 18,439 頂點與 36,874 三角形、保留擁有者核可的
軀幹四斷面 86/71/62/90，並確認手臂角度到達目標。
"""
import argparse
import json
import sys
