...
v9 純文生圖的 15、30、45 度提示主要落在正面、四分之三與側面三群。
本探針檢查幾何條件化後的臉部朝向是否隨 yaw 單調變化。

採用經自我驗證的鼻尖相對兩眼中點水平偏移，並以眼距正規化。
肩寬比與剪影面積的先前量測需進一步校準，因此沿用已驗證的鼻眼指標。
背面段的臉部偵測結果單獨記錄，提供朝向及可見性資訊。
"""
import re
import os
import sys
