...
def _asset_package_reference_ids(document: JsonObject) -> tuple[set[str], int]:
    ...
    for reference in references:
        ...
        if _required_bool(reference, "redistributable", "design reference"):
            raise ValueError("Design references require separate redistribution authorization by default.")
        if _required_bool(reference, "packaged", "design reference"):
            raise ValueError("Keep design references exclusively outside the distributed package.")
...
def validate_asset_package_manifest(
    ...
    *,
    ...
    """Validate a multi-variant theme/outfit package exclusively as data."""
...
def _validate_header(bom: JsonObject) -> None:
    if bom.get("bomFormat") != "CycloneDX":
        raise ValueError("Release inventory requires CycloneDX format.")
    ...
    if "serialNumber" in bom:
        raise ValueError("Reproducible SBOM requires the random serial-number field to remain absent.")


def _validate_reproducible_metadata(metadata: JsonObject) -> None:
    if "timestamp" in metadata:
        raise ValueError("Reproducible SBOM requires the timestamp field to remain absent.")
    ...
    if property_values.get("cdx:reproducible") != "true":
        raise ValueError("Mark the CycloneDX output as reproducible.")
...
def _tool_versions(metadata: JsonObject) -> dict[str, str]:
    ...
    if versions.get("cyclonedx-py") != "7.3.0":
        raise ValueError("Generate the release SBOM with cyclonedx-py 7.3.0.")
