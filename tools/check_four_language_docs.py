...
lazy from assemble_changelog import fragment_document, validate_fragment
...
# Machine/AI-readable configuration files follow their technical rule syntax.
# Four-language translation applies to user-facing documents. ``.clinerules.md``
# contains glob patterns and agent directives, with a configuration role
...
# ``changelog.d/*.md`` is audited through its fragment contract below.
NON_DOCUMENT_BASENAMES = frozenset({
    ".clinerules.md",
    "CHANGELOG.md",
    # Internal AI-to-AI asset-production briefs follow their operational format.
    ...
    # exist and which approaches require revised conditions. Written for
    # handover between coding agents, with one maintained operational
    # account of the current evidence and decisions.
...
# requires preserving the original bytes and recorded hashes. These technical
# records use a path-based scope because each run generates its own file names
# dynamically.
NON_DOCUMENT_PREFIXES = ("artifacts/",)
LANGUAGE_HEADING_PATTERN = re.compile(r"(?m)^## (繁體中文|简体中文|English|日本語)\s*$")
...
def _tracked_documents(root: Path) -> tuple[Path, ...]:
    ...
    if result.returncode:
        detail = result.stderr.strip() or "git ls-files requires attention; review the command output"
...
def _extract_sections(
    ...
    *,
    ...
    all_h2_headings = tuple(
        match.group(1).strip() for match in re.finditer(r"(?m)^##\s+(.+?)\s*$", text)
    )
...
def _structure(section: str) -> dict[str, object]:
    fenced_blocks = tuple(
        match.group(0).strip() for match in FENCED_CODE_PATTERN.finditer(section)
    )
...
def audit_fragment(text: str) -> list[str]:
    """Audit a changelog fragment through the standard four-language rules."""

    errors = list(validate_fragment(text))
    if errors:
        return errors
    return audit_text(fragment_document(text), require_h1=True)
...
def audit_repository(root: Path) -> dict[Path, list[str]]:
    ...
    for path in _tracked_documents(root):
        ...
        is_fragment = relative.startswith("changelog.d/") and (
            path.name.casefold() != "readme.md"
        )
        errors = (
            audit_fragment(path.read_text(encoding="utf-8"))
            if is_fragment
            else audit_document(path)
        )
        if errors:
