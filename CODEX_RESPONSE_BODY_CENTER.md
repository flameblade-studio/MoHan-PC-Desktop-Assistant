...
- x 座標的 ±20px 波動是「轉頭時身體在畫布中的自然水平偏移」，屬於預期的自然位移，
  應作為保留現狀的依據。
...
**判斷：這些「突跳」是轉頭時 3D 物體的自然非線性移動，屬於預期的自然位移，應維持現狀。**
...
**關鍵證據**：43 個 outlier 的範圍**全部位於臉部五官之外**（base/jaw/lips/iris/eyelid/brow）。
...
**素材已就緒，可直接進入下一階段。** DeepSeek 端將直接進入「實作 25 圖層渲染器並接入正式流程」階段。
