...
def _assert_checkbox_contract(app: QApplication) -> None:
...
def _assert_ai_wait_expression_contract(app: QApplication, window: CompanionWindow) -> None:
...
def _assert_portable_profile_and_features(window: CompanionWindow) -> None:
...
def _assert_character_surface(app: QApplication, window: CompanionWindow) -> None:
...
def _assert_expression_contract(window: CompanionWindow) -> None:
...
def _assert_ambient_expression_contract(window: CompanionWindow) -> None:
    ambient_choices = []
...
def _assert_smart_topmost_contract(window: CompanionWindow) -> None:
    z_order_calls = []
...
def _assert_native_z_order_contract(window: CompanionWindow) -> None:
    class FakeUser32:
        def __init__(self):
            self.calls = []
...
def _prepare_physics_layers(app: QApplication, window: CompanionWindow) -> None:
...
def _assert_physics_sources(window: CompanionWindow) -> None:
...
def _assert_dynamic_physics_rendering(window: CompanionWindow) -> None:
...
def _assert_attention_and_idle_contract(window: CompanionWindow) -> None:
...
def _assert_physics_tick_contract(window: CompanionWindow) -> None:
...
def _assert_physics_settings_contract(window: CompanionWindow) -> None:
...
def _assert_window_shell_contract(window: CompanionWindow) -> None:
...
def _assert_work_timer_contract(window: CompanionWindow) -> None:
    assert window.db.start_work()
    started_at = local_wall_time() - timedelta(seconds=65)
...
def _assert_voice_selection_contract(window: CompanionWindow) -> None:
    ...
    window.dashboard.voice_engine.setCurrentIndex(openai_index)
...
def _assert_transcription_contract(window: CompanionWindow) -> None:
    assert (
...
def _assert_model_selection_contract(window: CompanionWindow) -> None:
...
def _assert_realtime_settings_contract(window: CompanionWindow) -> None:
    assert (
...
def _assert_voice_controls_contract(window: CompanionWindow) -> None:
...
def _assert_reminder_controls_contract(window: CompanionWindow) -> None:
...
def _assert_realtime_text_contract(app: QApplication, window: CompanionWindow) -> None:
...
def _assert_todo_contract(app: QApplication, window: CompanionWindow) -> None:
...
def _assert_idea_contract(app: QApplication, window: CompanionWindow) -> None:
...
def _assert_memory_contract(app: QApplication, window: CompanionWindow) -> None:
...
def _assert_queued_speech_contract(app: QApplication, window: CompanionWindow) -> None:
...
def _assert_audio_viseme_contract(window: CompanionWindow) -> None:
...
def _assert_realtime_audio_shutdown_contract(window: CompanionWindow) -> None:
    class DummyStream:
        def __init__(self):
            self.aborts = 0
            self.closes = 0
...
def _assert_profile_customization_contract(app: QApplication, window: CompanionWindow) -> None:
...
def _capture_companion_previews(app: QApplication, window: CompanionWindow, preview: str) -> None:
...
def _capture_dashboard_previews(app: QApplication, window: CompanionWindow, preview: str) -> None:
    window.bubble.hide()
    window.dashboard.show()
    app.processEvents()
...
def _capture_memory_previews(app: QApplication, window: CompanionWindow, preview: str) -> None:
...
def _capture_idea_preview(app: QApplication, window: CompanionWindow, preview: str) -> None:
...
def _capture_previews(
    app: QApplication,
    window: CompanionWindow,
) -> None:
    preview = os.getenv("MOHAN_PREVIEW_PATH")
    if preview:
        _capture_companion_previews(app, window, preview)
        _capture_dashboard_previews(app, window, preview)
        _capture_memory_previews(app, window, preview)
        _capture_idea_preview(app, window, preview)


def _run_window_contracts(
    app: QApplication,
    window: CompanionWindow,
) -> None:
    _assert_ai_wait_expression_contract(app, window)
    _assert_portable_profile_and_features(window)
    _assert_character_surface(app, window)
    _assert_expression_contract(window)
    _assert_ambient_expression_contract(window)
    _assert_dashboard_topmost_contract(app, window)
    _assert_smart_topmost_contract(window)
    _assert_native_z_order_contract(window)
    _prepare_physics_layers(app, window)
    _assert_physics_sources(window)
    _assert_dynamic_physics_rendering(window)
    _assert_attention_and_idle_contract(window)
    _assert_physics_tick_contract(window)
    _assert_physics_settings_contract(window)
    _assert_dashboard_layout_contract(app, window)
    _assert_custom_platform_contract(window)
    _assert_window_shell_contract(window)
    _assert_work_timer_contract(window)
    _assert_voice_selection_contract(window)
    _assert_transcription_contract(window)
    _assert_model_selection_contract(window)
    _assert_realtime_settings_contract(window)
    _assert_voice_controls_contract(window)
    _assert_reminder_controls_contract(window)
    _assert_chat_zoom_contract(window)
    _assert_listener_state_contract(app, window)
    _assert_realtime_text_contract(app, window)
    _assert_chat_history_contract(window)
    _assert_todo_contract(app, window)
    _assert_idea_contract(app, window)
    _assert_memory_contract(app, window)
    _assert_queued_speech_contract(app, window)
    _assert_audio_viseme_contract(window)
    _assert_realtime_audio_shutdown_contract(window)
    _assert_profile_customization_contract(app, window)
    _capture_previews(app, window)


def run() -> None:
    with TemporaryDirectory(ignore_cleanup_errors=True) as tmp:
        _prepare_database(tmp)
        app = QApplication([])
        _assert_checkbox_contract(app)
        _assert_first_run_wizard(app, tmp)
        window = _create_window(app)
        _run_window_contracts(app, window)
        window.close()
        app.processEvents()
    print("UI_SMOKE_OK")
