...
- `expression_system.py` 繼續擁有表情選擇、優先級與持續時間；渲染器只依表情系統輸入呈現情緒。
- 現有 `VisemeFrame` 已提供離散嘴型、連續下顎開度與權重，可在不改變供應器契約下擴充。
- `app.py` 已有臉、眼睛、頭髮、袖子與飾品圖層，但臉部合成責任必須逐步移出這個組裝外殼。

### 目標架構

1. `face_rig.py`：獨立於 Qt 的唯讀資料模型，描述姿態、眼皮、眉毛、視線、紅暈、微笑、唇寬、唇圓、嘴部開度與下顎位移。
2. `face_motion.py`：把表情決策與 `VisemeFrame` 合成單一 `FaceMotionFrame`；擁有共同發音、平滑、限速與語音結束歸零規則。
3. `face_assets.py`：載入並驗證每個姿態的透明圖層、遮罩、錨點、局部網格及素材版本；只載入完整且通過保真驗證的素材。
4. `face_renderer.py`：定義小型 `FaceRendererPort`，並提供既有相容渲染器與新的參數化渲染器。兩者接收相同輸入，切換不改變語音、表情或資料庫。
5. `app.py`：只負責組裝、Qt 計時器及顯示最終畫面，共同發音與臉部變形規則由對應領域模組唯一擁有。

資料流固定為：語音供應器 → `lip_sync.py` → `VisemeFrame` → `face_motion.py` → `FaceMotionFrame` → `FaceRendererPort` → Qt 畫面。嘴型時鐘、表情仲裁器與動畫路徑各維持一套共用實作。
...
- 已完成合成的 TTS 可使用短距離、具上限的前視窗口，使下一個音素提早影響唇圓與唇寬，但畫面以實際播放起點為最早顯示時刻。
- Realtime 與麥克風串流使用因果平滑與目前／過去訊號，延遲只來自實際可用資料。
- 閉唇音、擦音、母音、靜音與語音結束都必須有明確狀態；最後的閉嘴事件具有最高終止權，晚到的音訊分析維持閉嘴狀態。
...
2. 三姿態素材：同時為 `front`、`lean`、`cheek` 建立眼皮、紅暈、嘴唇、嘴角與下顎圖層；三個姿態全部齊備時才標示整體完成。
3. 連續嘴型：加入唇寬、唇圓、開度、下顎與共同發音插值，完成繁中、簡中、英文、日文的代表音句測試。
4. 表情疊加：加入眉毛、眼角笑意、紅暈與眨眼的獨立混合，消除膚色覆蓋及表情／發音衝突。
5. 全姿態整合：一次接通 `front`、`lean`、`cheek` 與所有既有表情；每個姿態都需獨立素材稽核與實機驗收，並以單一版本一次上線。
...
- 臉部渲染平均低於 8 ms、P95 低於 16 ms、P99 低於 24 ms，並維持已保存的同機基準；有明確效益證據時依證據審查取捨。
- 語音結束後嘴型與聲音同時結束；重複、延遲或亂序完成事件皆維持閉嘴狀態。
- 眨眼時紅暈維持一致；說話結束後嘴角、眼線與嘴型回到權威狀態；不同姿態切換保持座標連續與單一輪廓。
- 角色五官、臉型、髮飾及服裝身分必須符合權威素材，並由使用者在實機以正常比例及放大檢視驗收。
- 記錄冷啟動、記憶體、CPU、JIT 開關對照及封裝容量；任何顯著退步都會阻止預設切換。

### 單一完整交付

本次工作一次完成領域資料模型、渲染介面、既有相容轉接器、三姿態素材清冊、連續嘴型、表情圖層與決定性測試。正式畫面與發布產物在全部三姿態完成後才切換。完成後提交單一 Draft PR，附新舊畫面差異、效能數據、素材容量及回退證據；只有完整通過全部門檻才進入實機驗收與發布。
...
- `expression_system.py` 继续拥有表情选择、优先级与持续时间；渲染器只依据表情系统输入呈现情绪。
- 现有 `VisemeFrame` 已提供离散嘴型、连续下颌开度与权重，可在不改变提供程序契约下扩展。
- `app.py` 已有脸、眼睛、头发、袖子与饰品图层，但脸部合成职责必须逐步移出这个装配外壳。

### 目标架构

1. `face_rig.py`：独立于 Qt 的只读数据模型，描述姿态、眼皮、眉毛、视线、红晕、微笑、唇宽、唇圆、嘴部开度与下颌位移。
2. `face_motion.py`：把表情决策与 `VisemeFrame` 合成单一 `FaceMotionFrame`；拥有协同发音、平滑、限速与语音结束归零规则。
3. `face_assets.py`：加载并验证每个姿态的透明图层、遮罩、锚点、局部网格及素材版本；只加载完整且通过保真验证的素材。
4. `face_renderer.py`：定义小型 `FaceRendererPort`，并提供现有兼容渲染器与新的参数化渲染器。两者接收相同输入，切换不改变语音、表情或数据库。
5. `app.py`：只负责装配、Qt 定时器及显示最终画面，协同发音与脸部变形规则由对应领域模块唯一拥有。

数据流固定为：语音提供程序 → `lip_sync.py` → `VisemeFrame` → `face_motion.py` → `FaceMotionFrame` → `FaceRendererPort` → Qt 画面。嘴型时钟、表情仲裁器与动画路径各维持一套共用实现。
...
- 已完成合成的 TTS 可使用短距离、具上限的前视窗口，使下一个音素提前影响唇圆与唇宽，但画面以实际播放起点为最早显示时刻。
- Realtime 与麦克风串流使用因果平滑与当前／过去信号，延迟只来自实际可用数据。
- 闭唇音、擦音、元音、静音与语音结束都必须有明确状态；最后的闭嘴事件具有最高终止权，迟到的音频分析维持闭嘴状态。
...
2. 三姿态素材：同时为 `front`、`lean`、`cheek` 建立眼皮、红晕、嘴唇、嘴角与下颌图层；三个姿态全部齐备时才标示整体完成。
3. 连续嘴型：加入唇宽、唇圆、开度、下颌与协同发音插值，完成繁中、简中、英文、日文的代表音句测试。
4. 表情叠加：加入眉毛、眼角笑意、红晕与眨眼的独立混合，消除肤色覆盖及表情／发音冲突。
5. 全姿态集成：一次接通 `front`、`lean`、`cheek` 与全部现有表情；每个姿态都需独立素材审计与真机验收，并以单一版本一次上线。
...
- 脸部渲染平均低于 8 ms、P95 低于 16 ms、P99 低于 24 ms，并维持已保存的同机基准；有明确效益证据时依据证据审查取舍。
- 语音结束后嘴型与声音同时结束；重复、延迟或乱序完成事件均维持闭嘴状态。
- 眨眼时红晕维持一致；说话结束后嘴角、眼线与嘴型回到权威状态；不同姿态切换保持坐标连续与单一轮廓。
- 角色五官、脸型、发饰及服装身份必须符合权威素材，并由用户在真机以正常比例及放大检视验收。
- 记录冷启动、内存、CPU、JIT 开关对照及封装容量；任何显著退步都会阻止默认切换。

### 单一完整交付

本次工作一次完成领域数据模型、渲染接口、现有兼容适配器、三姿态素材清单、连续嘴型、表情图层与确定性测试。正式画面与发布产物在全部三姿态完成后才切换。完成后提交单一 Draft PR，附新旧画面差异、性能数据、素材容量及回退证据；只有完整通过全部门槛才进入真机验收与发布。

## English

### Purpose and boundaries

This plan fully replaces whole-expression image switching with a continuously controlled, layered 2.5D face while preserving MoHan's stable behavior. Its implementation scope is a local layered renderer with the character's identity preserved; video, a 3D engine, a cloud renderer, and runtime generation remain outside that scope. Front, left-facing, and chin-rest poses, with every existing expression, blink, and viseme, must be completed in one delivery. The current renderer remains an immediate rollback path, and every pose ships together.

### Existing foundations

- `lip_sync.py` remains the single source of truth for the shared 50 Hz mouth timeline used by every speech provider.
- `expression_system.py` continues to own expression selection, priority, and duration; the renderer presents only emotion selected by that system.
- The existing `VisemeFrame` already carries a discrete viseme, continuous jaw aperture, and weight, so it can evolve while keeping provider contracts stable.
...
The fixed data flow is: speech provider → `lip_sync.py` → `VisemeFrame` → `face_motion.py` → `FaceMotionFrame` → `FaceRendererPort` → Qt display. The design keeps one shared mouth clock, one expression arbiter, and one provider-neutral animation path.

### Layers and deformation

Each supported pose separates base skin, left and right eyelids and eyeliner, left and right brows, irises/gaze, cheek blush, upper lip, lower lip, mouth corners, oral cavity, teeth/tongue masks, and the jaw/chin influence region. Prefer local control points and small triangular meshes. Optical flow may only generate or validate intermediate assets offline and remains an offline-only tool outside production runtime dependencies. Every layer uses one coordinate system, anchor manifest, and authoritative character-identity source.
...
- Fully synthesized TTS may use a short, bounded look-ahead window so the next phoneme can begin influencing lip rounding and width, but the real playback-start gate defines the earliest visual time.
- Realtime and microphone streams use causal smoothing with current and past signals only; latency derives only from current and past signals that are actually available.
- Bilabials, fricatives, vowels, silence, and speech completion all have explicit states. The final close event has terminal authority and late analysis preserves the final closed-mouth state.
...
5. All-pose integration: connect `front`, `lean`, `cheek`, and every existing expression in one change. Each pose still requires an independent asset audit and real-device acceptance, and they ship in one release.
6. Comparison and switch: the new renderer produces offline comparisons in tests and diagnostics, while user runtime uses one renderer. It becomes the default once, after every gate passes, and the legacy renderer remains available for at least one complete release-candidate cycle.
...
- Face rendering remains below 8 ms mean, 16 ms P95, and 24 ms P99, and maintains the stored same-machine baseline; evidence of a clear benefit supports any reviewed tradeoff.
- Mouth motion ends with audio; duplicate, late, or out-of-order completion events preserve the final closed-mouth state.
- Blinks preserve blush; after speech, mouth corners, eyeliner, and visemes return to their authoritative state; pose transitions keep continuous coordinates and one contour.
...
- `expression_system.py` は表情の選択、優先順位、継続時間を引き続き所有し、レンダラーは表情システムが選択した感情だけを表示します。
- 既存の `VisemeFrame` は離散ビセーム、連続的な顎の開度、重みをすでに持つため、プロバイダー契約を維持したまま拡張できます。
...
3. `face_assets.py`：各姿勢の透過レイヤー、マスク、アンカー、局所メッシュ、素材バージョンを読み込み検証します。完全性と品質の検証を通過した素材だけを読み込みます。
...
データフローは、音声プロバイダー → `lip_sync.py` → `VisemeFrame` → `face_motion.py` → `FaceMotionFrame` → `FaceRendererPort` → Qt 表示に固定します。口形時計、表情調停器、プロバイダー共通アニメーション経路をそれぞれ一つに統一します。
...
- 合成済み TTS は、次の音素が唇の丸みと幅へ早めに影響する、短く上限付きの先読み窓を使用できます。ただし、実際の再生開始ゲートを映像表示の最早時刻とします。
- Realtime とマイクストリームは現在および過去の信号だけによる因果平滑化を使い、遅延は実際に利用可能な現在および過去の信号だけから決まります。
...
2. 三姿勢素材：`front`、`lean`、`cheek` のまぶた、赤み、唇、口角、顎レイヤーを同時に構築し、三姿勢がすべて揃った時点で全体完成とします。
...
6. 比較と切り替え：新レンダラーはテストと診断でオフライン比較を生成し、ユーザー実行時は一つのレンダラーを使用します。すべてのゲート通過後に一度だけ既定値へ切り替え、現行レンダラーを少なくとも一つの完全なリリース候補サイクル中はロールバック用に維持します。

### 妥協しない受け入れゲート

- 既存の完全回帰、アーキテクチャ、四言語、セキュリティ、SBOM、パッケージ、公開内容監査がすべて合格します。
- 50 Hz のキュー頻度を維持し、再生後の最初のキューは 40 ms 以内、長時間音声の平均間隔誤差は 3 ms 以内を維持します。
- 顔レンダリングは平均 8 ms 未満、P95 16 ms 未満、P99 24 ms 未満とし、保存済み同一環境基準を維持し、明確な利点の証拠がある場合はその取捨を審査します。
...
- コールドスタート、メモリ、CPU、JIT 有効／無効比較、パッケージ容量を記録し、既定有効化は各指標が受入基準を満たした後に行います。
