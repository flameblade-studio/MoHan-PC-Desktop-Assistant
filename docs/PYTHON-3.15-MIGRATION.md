...
執行環境基準：**僅限 CPython 3.15.0rc1**。完成遷移後，墨寒統一使用一套
...
墨寒採用「新版能力可安全適用時，統一採用新版寫法」原則。已導入 PEP 810
...
`sentinel` 已加入治理測試；目前舊式 `object()` 哨兵候選數為 0，未來一旦
「未傳值」與 `None` 必須區分，就必須使用內建 `sentinel`。
開頭比對已改用語意明確的 `re.prefixmatch()`，並由稽核維持語意明確的 `re.prefixmatch()` 單一路徑。
...
執行環境；完整維持 PyInstaller 的隔離邊界。`PYTHON_JIT=0` 僅用於效能與
相容性對照。PEP 831 Frame Pointer、Windows tail-calling interpreter、
PEP 782／788 與其他 C API 項目在 runtime report、官方輪子與 CI 的真實邊界驗證。PEP 829、728、747、800 與標準庫新增 API
目前的等價且有價值使用點數為 0；未來功能符合用途時重新評估並採用。

兩輪各 20,000 次表情、物理與嘴型整合壓測均通過；JIT 關閉／開啟分別耗時
24.639／25.068 秒，工作集成長 10.62／12.94 MB。此負載的 JIT 證據以這組實測結果為準，
...
JSONL、pstats、GC／配置／執行緒資料與 SHA-256，原始二進位取樣流維持在內部隔離範圍。
隔離的 `pip-audit` 2.10.1 與 CycloneDX 7.3.0 產生器使用 3.14.7；
...
0.86–0.98 倍（中位數 0.97 倍）；2,000 個 50 Hz 嘴型分析節拍
為 1.45–1.65 倍（中位數 1.48 倍）。每一輪的決策、強度校驗和與
A／I／U／E／O 結果完全相同。結果顯示 JIT 效益取決於熱路徑，證據範圍是受測
嘴型分析路徑加速；表情仲裁與整體程式各自保留獨立效能證據門檻。
...
| PEP 814 `frozendict` | 已用於全域與巢狀設定 | 新增靜態映射統一採用不可變結構 |
| PEP 661 `sentinel` | 執行期已驗證；目前舊式哨兵候選數為 0 | 未傳值與明確 `None` 不同時使用 |
...
| PEP 803／820／793 Stable ABI 與 C API 現代化 | 已驗證 Qt ABI3 輪子；第一方 C 擴充數為 0 | 2.3.0 RC2 的允收集合限定為正式 ABI3 Qt 輪子 |
| PEP 782 與 PEP 788 C API | 適用狀態：自有 C 擴充數為 0 | 加入原生程式碼前重新評估 |
| PEP 829 啟動設定 | 目前封裝桌面入口的需求數為 0 | 墨寒成為可安裝套件或外掛主機時重新評估 |
| PEP 728／747／800 型別功能 | 目前等價正式模型數為 0 | 引入型別化外掛負載或 type-form API 時採用 |
...
运行环境基准：**仅限 CPython 3.15.0rc1**。完成迁移后，墨寒统一使用一套
...
墨寒遵循“新版能力能够安全适用时，统一采用新版写法”的原则。已导入 PEP 810
...
`sentinel` 已加入治理测试；目前旧式 `object()` 哨兵候选数为 0，未来一旦
“未传值”与 `None` 必须区分，就必须使用内建 `sentinel`。
开头匹配已改用语义明确的 `re.prefixmatch()`，并由审计保持语义明确的 `re.prefixmatch()` 单一路径。
...
运行环境；完整保持 PyInstaller 隔离边界。`PYTHON_JIT=0` 仅用于性能与兼容性
对照。PEP 831 Frame Pointer、Windows tail-calling interpreter、PEP 782／788
及其他 C API 项目在 runtime report、官方轮子与 CI 的真实边界验证。PEP 829、728、747、800 与标准库新增 API 目前等价且
有价值的使用点数为 0；未来功能符合用途时重新评估并采用。

两轮各 20,000 次表情、物理与口型整合压力测试均通过；JIT 关闭／开启分别耗时
24.639／25.068 秒，工作集增长 10.62／12.94 MB。此负载的 JIT 证据以这组实测结果为准，
...
JSONL、pstats、GC／配置／线程数据与 SHA-256，原始二进制采样流保持在内部隔离范围。
隔离的 `pip-audit` 2.10.1 与 CycloneDX 7.3.0 生成器使用 3.14.7；
...
0.86–0.98 倍（中位数 0.97 倍）；2,000 个 50 Hz 口型分析节拍
为 1.45–1.65 倍（中位数 1.48 倍）。每一轮的决策、强度校验和与
A／I／U／E／O 结果完全相同。结果显示 JIT 收益取决于热路径，证据范围是受测
口型分析路径加速；表情仲裁与整个程序各自保留独立性能证据关卡。
...
| PEP 814 `frozendict` | 已用于全局与嵌套配置 | 新增静态映射统一采用不可变结构 |
| PEP 661 `sentinel` | 运行时已验证；目前旧式哨兵候选数为 0 | 未传值与明确 `None` 不同时使用 |
...
| PEP 803／820／793 Stable ABI 与 C API 现代化 | 已验证 Qt ABI3 轮子；第一方 C 扩展数为 0 | 2.3.0 RC2 的接收集合限定为正式 ABI3 Qt 轮子 |
| PEP 782 与 PEP 788 C API | 适用状态：自有 C 扩展数为 0 | 添加原生代码前重新评估 |
| PEP 829 启动配置 | 当前打包桌面入口的需求数为 0 | 墨寒成为可安装包或插件主机时重新评估 |
| PEP 728／747／800 类型功能 | 当前等价正式模型数为 0 | 引入类型化插件负载或 type-form API 时采用 |
...
Runtime baseline: **CPython 3.15.0rc1 only**. MoHan uses one unified
...
MoHan follows the rule “adopt the new idiom uniformly when a capability is safe and applicable.” Integrated work includes project-wide explicit PEP 810
...
by CI; the legacy `object()` sentinel candidate count is 0 today, and a future API that must
distinguish omission from `None` must use the built-in `sentinel`.
Prefix matching now uses the explicit `re.prefixmatch()` API, and the audit maintains `re.prefixmatch()` as the single explicit path.
...
default while fully preserving PyInstaller isolation. `PYTHON_JIT=0` is used only
...
at their real runtime-report, official-wheel, and CI boundaries. PEP 829, 728, 747, 800, and new standard-library APIs have an equivalent valuable use count of 0 today and are reassessed when a feature fits.
...
This workload uses those measurements as its JIT evidence, while product policy keeps JIT on
...
allocation,
thread, and SHA-256 evidence, while the raw binary stream remains inside the isolated internal scope. The isolated
...
(0.97x median), while 2,000 50 Hz
...
hot-path dependent: the evidence scope is acceleration of the
measured viseme path; expression arbitration and the complete application retain
separate performance-evidence gates.
...
| PEP 814 `frozendict` | Integrated for global and nested configuration | New static mappings uniformly use immutable structures |
| PEP 661 `sentinel` | Runtime verified; legacy sentinel candidate count is 0 | Use when omission and explicit `None` differ |
...
| PEP 803/820/793 Stable ABI and C API modernization | Qt ABI3 wheels verified; first-party C-extension count is 0 | Accept official ABI3 Qt wheels in 2.3.0 RC2 |
| PEP 782 and PEP 788 C APIs | Applicability: MoHan-owned C-extension count is 0 | Reassess before adding native code |
| PEP 829 startup configuration | Current packaged-desktop requirement count is 0 | Reassess if MoHan becomes an installable package or plugin host |
| PEP 728/747/800 typing features | Current equivalent production-model count is 0 | Adopt with typed plugin payloads or type-form APIs |
...
実行環境の基準は **CPython 3.15.0rc1 のみ**です。移行後の墨寒は、一つの Python 実行環境へ統一します。アップストリームの完全な確認項目は、公式の
...
墨寒は「新機能を安全に適用できる場合、新方式へ統一する」方針です。PEP 810
...
`sentinel` は CI で管理しています。現在の旧式 `object()` センチネル候補数は 0 で、
将来「未指定」と `None` を区別する API では組み込み `sentinel` を使います。
先頭一致には明示的な `re.prefixmatch()` を使用し、監査によって `re.prefixmatch()` の明示的な単一路径を維持します。
...
PyInstaller の隔離を完全に維持して JIT を既定で有効にします。`PYTHON_JIT=0` は
性能・互換性比較だけに使用します。PEP 831 Frame Pointer、Windows
tail-calling interpreter、PEP 782／788 などの C API は runtime report、
公式 wheel、CI の実境界で検証します。PEP 829、728、
747、800 と標準ライブラリの新 API は、現在の価値ある用途数は 0 で、用途が生じた時点で再評価します。

表情、物理、口形の 20,000 回統合耐久試験を二回通過しました。JIT 無効／有効は
24.639／25.068 秒、作業セット増加は 10.62／12.94 MB で、この負荷の JIT 証拠はこの実測結果を使用し、製品方針により既定で有効にし、互換性用に
...
バイナリストリームは内部隔離範囲に維持します。隔離された `pip-audit` 2.10.1 と
...
表情調停で 0.86～0.98 倍（中央値 0.97 倍）、2,000 回の
50 Hz 口形解析で 1.45～1.65 倍（中央値 1.48 倍）でした。各回の判断、強度
チェックサム、A／I／U／E／O の結果は完全に一致しました。JIT の効果は
ホットパスに依存し、今回の加速証拠は測定した口形解析経路を対象とします。
表情調停とアプリ全体は、それぞれ独立した性能証拠ゲートを維持します。
...
| PEP 814 `frozendict` | グローバル・入れ子設定へ導入済み | 新しい静的マッピングを不変構造へ統一 |
| PEP 661 `sentinel` | 実行時検証済み。旧式候補数は 0 | 未指定と明示的な `None` が異なる場合に使用 |
...
| PEP 803／820／793 Stable ABI と C API 現代化 | Qt ABI3 wheel を検証済み。自製 C 拡張数は 0 | 2.3.0 RC2 は公式 ABI3 Qt wheel を受け入れる |
| PEP 782 と PEP 788 C API | 非該当：墨寒に自製 C 拡張数は 0 | ネイティブコード追加前に再評価 |
| PEP 829 起動設定 | 現在のデスクトップ入口での必要数は 0 | インストール可能パッケージ／プラグインホスト化時に再評価 |
| PEP 728／747／800 型機能 | 現在の同等本番モデル数は 0 | 型付きプラグイン負荷や type-form API 導入時に採用 |
