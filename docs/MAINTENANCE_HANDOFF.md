...
For device acceptance, confirm both framings speak without corruption and with audio/lip motion, the display remains a single character, text chat works, camera activity/waving receives visible responses, autonomous and manual wardrobes coexist, and both lock-duration arrows work at minimum and maximum. If a discrepancy remains, open a GitHub issue or follow-up PR and record it in the traceable project history.

1. On another computer, obtain main and the newest open PR, then read this document and the PR description.
2. Use the exact GitHub Actions job log to identify the exact cause, make the smallest direct repair, and push it.
3. When all required checks pass, merge the PR and verify the relationship among main, tag, and published release.
4. Write the merge SHA, verification results, pending device items, and next step back into this document, then commit it.
...
2. 正確な GitHub Actions ジョブログで正確な原因を確認し、最小の直接修正を行ってプッシュします。
3. 必要なチェックがすべて通ったら PR をマージし、main、タグ、公開版の対応を確認します。
4. マージ SHA、検証結果、完了待ちの実機項目、次の手順をこの文書へ戻してコミットします。
