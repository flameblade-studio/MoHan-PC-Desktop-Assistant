...
- `v2.2.0-rc.N` 另在原生 runner 產生兩種 macOS 架構的 DMG 與 Linux
  AppImage，並從完成的安裝包執行
...
- `v2.2.0-rc.N` 会在原生 runner 生成两种 macOS 架构的 DMG 与 Linux
  AppImage，并从完成的安装包执行
...
- The `v2.2.0-rc.N` line also builds both macOS architecture DMGs and the Linux
...
- `v2.2.0-rc.N` 系列では、ネイティブ runner で macOS 両アーキテクチャの
