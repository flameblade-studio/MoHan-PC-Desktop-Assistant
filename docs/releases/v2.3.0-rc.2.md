...
- 修正 Windows 縮小後顯示空白文件圖示：新版工作列圖示取自與鐵人賽形象一致的正面墨寒半身像，提供 16 至 256 像素共十種原生 Windows 圖示影格；EXE 捷徑改用安裝目錄內的圖示，MSI 與執行中視窗共用固定工作列身分，視窗也會在原生旗標確定後套用圖示。舊版五官輪廓的全身像已從公開資產移除。
...
- 修复 Windows 最小化后显示空白文档图标的问题：新版任务栏图标取自与铁人赛形象一致的正面墨寒半身像，提供 16 至 256 像素共十种原生 Windows 图标帧；EXE 快捷方式改用安装目录内的图标，MSI 与运行中的窗口共用固定任务栏身份，窗口也会在原生标志确定后应用图标。旧版五官轮廓的全身像已从公开资源中删除。
...
- Fixes the blank-document taskbar icon shown after minimizing on Windows. The new icon uses the front-facing canonical MoHan half-body identity shared with the Ironman visuals and supplies ten native Windows frames from 16 through 256 pixels. EXE shortcuts use the installed icon, MSI and running windows share one taskbar identity, and each window applies its icon after native flags are final. The obsolete full-body asset with the earlier facial identity is removed from public assets.
...
- Windows で最小化した後に空白文書アイコンが表示される問題を修正します。新しいタスクバーアイコンは、鉄人賽のビジュアルと同じ正面向きの墨寒半身像を使い、16～256 ピクセルのネイティブ Windows アイコンを十段階で収録します。EXE ショートカットはインストール先のアイコンを参照し、MSI と実行中ウィンドウは同じタスクバー ID を共有し、ウィンドウはネイティブフラグ確定後にアイコンを適用します。旧版の顔立ちを持つ全身素材は公開資産から削除しました。
