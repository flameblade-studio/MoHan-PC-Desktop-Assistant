...
>
> **DLC テーマの導入は三手順**：Ko-fi で支援後、謝礼の案内に従って `.mohan-theme` ファイルを一つダウンロードします。コントロールパネルの「雲裳閣」タブで「ファイルを1つアップロード」からそのファイルを選び、プレビューで確認したら右下の「設定を保存」で正式に適用します。キャンセルすれば元のテーマに戻ります。テーマファイルはコードを含まず、解凍も不要で、アップグレード後もそのまま使えます。
