...
2. 執行 `MoHan-Desktop-Assistant-<version>.exe`（`<version>` 為版本號，實際檔名依你從
   [Releases](https://github.com/flameblade-studio/MoHan-PC-Desktop-Assistant/releases)
   下載的版本而定）。
3. 在首次設定精靈選擇「繁體中文（台灣）」，並確認助理名稱、稱呼、組織、
   工作類型與喚醒詞。
...
2. 运行 `MoHan-Desktop-Assistant-<version>.exe`（`<version>` 为版本号，实际文件名取决于你从
   [Releases](https://github.com/flameblade-studio/MoHan-PC-Desktop-Assistant/releases)
   下载的版本）。
...
2. Run `MoHan-Desktop-Assistant-<version>.exe` (`<version>` is the release
   number; the actual file name depends on the version you downloaded from
   [Releases](https://github.com/flameblade-studio/MoHan-PC-Desktop-Assistant/releases)).
...
2. `MoHan-Desktop-Assistant-<version>.exe` を実行します（`<version>` は版番号で、実際のファイル名は
   [Releases](https://github.com/flameblade-studio/MoHan-PC-Desktop-Assistant/releases)
   からダウンロードした版によって異なります）。
3. 初回設定で「日本語」を選び、アシスタント名、呼び名、組織、作業内容、
   ウェイクワードを確認します。
4. 新規利用者は Windows 本機音声を既定で利用できます。クラウド AI を使う
   場合は、設定画面に利用者自身の OpenAI API キーを入力します。
5. 既定モデル：
   - 文字：`gpt-5.6-luna`
   - Realtime：`gpt-realtime-2.1-mini`
   - 文字起こし：`gpt-4o-mini-transcribe`
   - TTS：`gpt-4o-mini-tts`
6. EXE と同梱フォルダーを分離しないでください。
