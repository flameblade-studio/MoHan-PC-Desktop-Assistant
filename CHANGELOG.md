...
### 未發布 — 淡雅妝獨立繪製（2026-09-03）

* 內建「淡雅」不再把「原妝」alpha 乘上固定比例，而是使用 31 個 silhouette 各自繪製的 `eyes`／`cheeks`／`lips` 三槽素材；封裝器會分別裁掉安全區外像素、記錄逐剪影裁切數並以 `verify_makeup_layers` 驗證密封包。回歸測試確認淡雅妝具有原妝沒有的唇部像素，且 100% 濃度下眼周深色像素仍較少。
...
### 未发布 — 淡雅妆独立绘制（2026-09-03）

* 内置「淡雅」不再把「原妆」alpha 乘以固定比例，而是使用针对 31 个 silhouette 分别绘制的 `eyes`／`cheeks`／`lips` 三槽素材；封装工具会分别裁掉安全区外像素、记录每个剪影的裁切数量，并用 `verify_makeup_layers` 验证密封包。回归测试确认淡雅妆含有原妆没有的唇部像素，并且在 100% 浓度下眼周深色像素仍然更少。
...
### Unreleased — independently painted light makeup (2026-09-03)

* The built-in `light` look no longer multiplies the `classic` alpha by a fixed ratio. It uses separately painted `eyes`/`cheeks`/`lips` assets for all 31 silhouettes; the assembler crops each set against the safe regions, records per-silhouette crop counts, and verifies the sealed archive with `verify_makeup_layers`. Regression coverage proves that light has lip pixels absent from classic and still has fewer dark eye-area pixels at 100% intensity.
...
### 未リリース — 淡めメイクを個別制作（2026-09-03）

* 内蔵の淡めメイクは、基本メイクの alpha に固定比率を掛ける方式を廃止し、31 silhouette ごとに個別制作した `eyes`／`cheeks`／`lips` 素材を使用します。組立ツールは各素材を安全領域で切り落とし、silhouette ごとの切り落とし数を記録し、封止済みパックを `verify_makeup_layers` で検証します。回帰テストでは淡めメイクに基本メイクにはない唇のピクセルがあり、濃さ 100% でも目元の暗色ピクセルが少ないことを確認します。
