...
- 檔名使用穩定的小寫 kebab-case，例如 `issue-140-migration.md`；未發布的 `CHANGELOG.md` 內容由 Release Please 與組裝器管理。
- Release Please 先在 release PR 產生版本標題，接著由 `tools/assemble_changelog.py --version <version>` 依檔名排序組裝；成功寫入後才刪除片段。`--dry-run` 只預覽。

本次遷移的既有三個變更保留原文，使用四個語言小節的相容格式；組裝時只把四語標題與條列重新排成斜線格式，文字逐字保留。組裝器與稽核同時支援這種格式。片段檔內容限於變更說明，憑證、個人資料與未驗證的發布承諾維持在片段之外。
...
- 文件名使用稳定的小写 kebab-case，例如 `issue-140-migration.md`；未发布的 `CHANGELOG.md` 内容由 Release Please 与组装器管理。
- Release Please 先在 release PR 产生版本标题，接着由 `tools/assemble_changelog.py --version <version>` 按文件名排序组装；成功写入后才删除片段。`--dry-run` 只预览。

本次迁移的既有三个变更保留原文，组装时只把四语标题与列表项重新排列为斜线格式，文字逐字保留。组装器与审计同时支持这种格式。片段文件内容限于变更说明，凭据、个人资料与未经验证的发布承诺维持在片段之外。
...
- Use a stable lowercase kebab-case filename such as `issue-140-migration.md`; leave the unreleased part of `CHANGELOG.md` to the Release Please assembly flow.
...
- `issue-140-migration.md` のような安定した小文字 kebab-case のファイル名を使い、`CHANGELOG.md` の未公開部分は Release Please の組み立てフローで管理します。
...
今回移行した三つの変更は、元の文言を四言語の小見出しによる互換形式で保持します。組み立て時は四言語の見出しと箇条書きをスラッシュ行へ並べ替えるだけで、各語を保持します。組み立てツールと監査はこの形式にも対応します。フラグメントの内容は変更説明に限定し、認証情報、個人データ、未検証の公開約束はフラグメントの外に保持します。
