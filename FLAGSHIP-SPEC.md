...
3. 攝影機：由使用者主動啟用、狀態持續可見、只在本機處理，錄影功能須由使用者明確啟動。
...
- Realtime、轉錄、TTS、嘴型同步、動畫、物理效果、表情選擇、拖曳行為、提醒、任務時序及遷移須維持或提升既有品質。
- 封裝後執行檔必須通過自我測試及重複的乾淨設定檔 smoke test。
- 既有設定檔遷移必須保留身分、模型、語音、記憶、對話、工作紀錄、提醒、動畫及個人設定。
- 遠端、攝影機、雲端及 Home Assistant 元件須提供各自獨立的啟用控制；離線桌面核心須持續運作。
...
3. 摄像头：由用户主动启用、状态持续可见、只在本地处理，录制功能须由用户明确启动。
...
- Realtime、转录、TTS、嘴型同步、动画、物理效果、表情选择、拖动行为、提醒、任务时序及迁移须维持或提升现有质量。
...
3. カメラ：ユーザーが明示的に有効化し、状態を継続表示し、ローカル処理に限定します。録画機能はユーザーの明示操作で開始します。
...
- Realtime、文字起こし、TTS、リップシンク、アニメーション、物理効果、表情選択、ドラッグ動作、リマインダー、タスクのタイミング、移行は既存品質を維持または向上します。
- パッケージ化した実行ファイルが、自己テストと反復するクリーンプロファイル smoke test に合格します。
- 既存プロファイルの移行で、アイデンティティ、モデル、音声、記憶、会話、作業記録、リマインダー、アニメーション、個人設定を維持します。
- リモート、カメラ、クラウド、Home Assistant の各コンポーネントを個別の有効化制御を備え、オフラインデスクトップコアは継続して動作します。
